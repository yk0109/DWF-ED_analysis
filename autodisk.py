...
#drawCircles  generateKernel generateAvg ndimage array_average  calculate_radius generate_vectors cal_copy
#mask_arrange loadmat readDatadat gauss_fit generate_blank_data create_ring_matrix

def visual(image, outdir=None, point=None, colorbar=False, normalization=False, title=None, color=None, mask=None):
    """
    ...
    if color is not None:
        im = ax.imshow(image_out,cmap=color)  # viridis gray
    else:
...
def generateAdf(data,in_rad,out_rad,center=None,angle=None,show_mask=False,outdir=None,dataformat='np.float32', imagedata=False): 
    ...
    adf = np.sum(data * mask_img, axis=(-2, -1))
    plt.imshow(adf,cmap='gray')
    plt.title(f'ADF-{adf.shape}')
    plt.axis('off')
    if outdir is not None:
...
def drawCircles(ori_pattern,blobs_list,r,lwide=2,outdir=None,title=None,xylim=None,text=False, points=None, sort=False, color_groups=None):
    """
    ...
    # Define color mapping based on group sizes
    # color_groups[i] specifies how many circles should use color i
    if color_groups is not None:
        # Use predefined colors for different groups
        predefined_colors = ['red', 'blue', 'green', 'orange', 'purple', 'brown', 'pink', 'cyan', 'yellow', 'magenta', 
                             'lime', 'navy', 'teal', 'coral', 'gold', 'indigo', 'salmon', 'olive', 'maroon', 'turquoise']
        # Create color mapping: each value in color_groups specifies the number of circles for that color
        color_index_map = {}  # Maps circle index to color
        current_index = 0
        
        for group_idx, num_circles in enumerate(color_groups):
            color = predefined_colors[group_idx % len(predefined_colors)]
            # Assign the same color to the specified number of circles
            for j in range(num_circles):
                if current_index < len(blobs_list):
                    color_index_map[current_index] = color
                    current_index += 1
                else:
                    break
    ...
    for q, blob in enumerate(blobs_list):
        ...
        if color_groups is not None and q in color_index_map:
            circle_color = color_index_map[q]
        else:
            circle_color = 'red'  # Default color for unassigned circles
        
        c = plt.Circle((x, y), r, color=circle_color, linewidth=lwide, fill=False)
        ax.add_patch(c)
        if text==True:
    ...
    # Set title and axis off (moved outside loop)
    if title is not None:
        plt.title(title)
    plt.axis('off')
...
def generate_blank_data(data_ori, size, method='fast'):
    '''
    对四维数组进行空间邻域平均，每个位置使用周围 2*size × 2*size 区域的图片进行平均。
    边界处仅平均实际存在的数据。

    Parameters
    ----------
    data_ori : 4D numpy array
        输入的四维数组，形状为 (H, W, img_h, img_w)，其中前两维是位置索引，后两维是图片形状。
    size : int
        邻域大小参数，将平均每个位置周围 2*size × 2*size 区域的图片。
    method : str, optional
        计算方法，'fast' 使用优化算法（默认），'accurate' 使用精确算法。

    Returns
    -------
    data_avg : 4D numpy array
        平均后的四维数组，形状与 data_ori 一致。

    Example
    -------
    假设 data_ori 形状为 (10, 10, 256, 256)，size=2
    对于位置 (0, 0)，将平均 (0,0), (0,1), (1,0), (1,1) 四个位置的图片
    对于位置 (5, 5)，将平均 (3,3) 到 (7,7) 共 25 个位置的图片
    '''
    H, W, img_h, img_w = data_ori.shape
    
    if method == 'fast':
        # 优化方法：使用cumsum实现快速滑动窗口平均
        # 在前两个维度上进行pad，以便处理边界
        from scipy.ndimage import uniform_filter
        
        # 对每个像素位置单独处理
        data_avg = np.zeros_like(data_ori, dtype=np.float64)
        
        # 将四维数据重塑为 (H*W, img_h*img_w) 进行处理
        data_reshaped = data_ori.reshape(H, W, -1)
        
        # 对前两个维度进行滑动平均
        for k in range(data_reshaped.shape[2]):
            # 使用uniform_filter对每个像素通道进行滑动平均
            data_avg[:, :, k // img_w, k % img_w] = uniform_filter(
                data_reshaped[:, :, k], 
                size=2*size, 
                mode='constant', 
                cval=0.0
        ...
        weight = uniform_filter(weight, size=2*size, mode='constant', cval=0.0)
        weight = np.maximum(weight, 1e-10)  # 避免除零
        
        # 应用权重修正
        data_avg = data_avg / weight[:, :, np.newaxis, np.newaxis]
        ...
        # 遍历每个位置
        print(f"Processing {H}x{W} positions...")
        for i in range(H):
            if i % 10 == 0:
                print(f"Progress: {i}/{H}")
            for j in range(W):
                # 计算邻域范围，确保不超出边界
                i_start = max(0, i - size)
                i_end = min(H, i + size)
                j_start = max(0, j - size)
                j_end = min(W, j + size)
                
                # 提取邻域内的所有图片并计算平均
                neighborhood = data_ori[i_start:i_end, j_start:j_end, :, :]
                data_avg[i, j, :, :] = np.mean(neighborhood, axis=(0, 1))
...
def histogram_analysis(arr1, bin_width=0.01, lim=[0,0.5,0,80,200], plot=True, plot_fit=True, out_dir=None):
    ...
    hist = hist[mask]
    if plot_fit==True:
        params, covariance = curve_fit(poly_func, bin_centers, hist, maxfev=10000)
    else:
        params = [1, 1, 1]
    min_fit = lim[0]
    max_fit = lim[1]
...
def cal_copy(value, X, Y, num_copy):
    if num_copy <= 0 or num_copy % 2 == 0:
        raise ValueError("num_copy must be a positive odd integer.")
    ...
    #print(value,y,x)
    num = num_copy//2
    value_set = matrix[max(0, y-num):min(Y, y+num+1),
                       max(0, x-num):min(X, x+num+1)]
    return value_set

def data_cal_copy(data_ori, value, X, Y, num_copy):
    HC_sub = []
    value_set = cal_copy(value,X,Y,num_copy)
    #print(value_set)
    rows, cols = value_set.shape
    if rows == 0 or cols == 0:
        raise ValueError(f"No valid neighborhood found for value={value} with num_copy={num_copy}.")
    for value_x in range(rows):
        for value_y in range(cols):
            HC_sub.append(data_ori[int(value_set[value_x, value_y])])
    pattern_shape = tuple(data_ori.shape[1:])
    HC_sub = np.array(HC_sub).reshape(rows, cols, *pattern_shape)
    data = generateAvg(HC_sub, normalization=True)
    return data

def data4D_cal_copy(test_position, region_size, normalize=False, step=1):
    """
    对 4D-STEM 数据在前两个扫描维度上做滑动窗口区域叠加。

    Parameters
    ----------
    test_position : np.ndarray
        4D 数组，形状为 (scan_x, scan_y, qx, qy)
    region_size : int or tuple[int, int]
        滑动窗口大小，如 5 或 (5, 5)
    normalize : bool
        是否将每个新点的总强度归一化到 1
    ...
    new_data : np.ndarray
        新的 4D 数组
    """
    data = np.asarray(test_position)

    if data.ndim != 4:
        raise ValueError(f"test_position 必须是 4 维数组，当前是 {data.ndim} 维")

    if isinstance(region_size, int):
        rx, ry = region_size, region_size
    else:
        rx, ry = region_size

    if isinstance(step, int):
        sx_step, sy_step = step, step
    else:
        sx_step, sy_step = step

    sx, sy, qx, qy = data.shape

    if rx > sx or ry > sy:
        raise ValueError("region_size 不能大于扫描维度")
    if sx_step <= 0 or sy_step <= 0:
        raise ValueError("step 必须为正整数")

    acc_dtype = np.result_type(data.dtype, np.float64)

    # 前两个维度做积分图，加速滑动窗口求和
    prefix = np.zeros((sx + 1, sy + 1, qx, qy), dtype=acc_dtype)
    prefix[1:, 1:] = data.astype(acc_dtype, copy=False).cumsum(axis=0).cumsum(axis=1)
    ...
    new_data = summed[::sx_step, ::sy_step]

    if normalize:
        total = new_data.sum(axis=(-2, -1), keepdims=True)
        total[total == 0] = 1
...
def rename_arina_dataset(master_path, new_prefix):
    """
    重命名一套 Arina 数据，并同步更新 master.h5 内的外部链接。
    ...
    ->
    sampleA_master.h5
    sampleA_data_000001.h5
    sampleA_data_000002.h5
    """
    ...
    with h5py.File(master_path, "r") as f:
        ...
        for key in data_group.keys():
            ...
            if not isinstance(link, h5py.ExternalLink):
                continue
            new_data_name = f"{new_prefix}_{key}.h5"
            updates.append((key, link.filename, link.path, new_data_name))

    for _, old_data_name, _, new_data_name in updates:
        old_file = folder / old_data_name
        new_file = folder / new_data_name
        if not old_file.exists():
            raise FileNotFoundError(f"找不到数据文件: {old_file}")
        if new_file.exists():
            raise FileExistsError(f"目标文件已存在: {new_file}")

    new_master_path = folder / f"{new_prefix}_master.h5"
    if new_master_path.exists():
        raise FileExistsError(f"目标 master 已存在: {new_master_path}")

    for _, old_data_name, _, new_data_name in updates:
        (folder / old_data_name).rename(folder / new_data_name)

    with h5py.File(master_path, "r+") as f:
        data_group = f["entry"]["data"]
        for key, _, link_path, new_data_name in updates:
            del data_group[key]
            data_group[key] = h5py.ExternalLink(new_data_name, link_path)

    master_path.rename(new_master_path)
...
def angle_range(center, ctr, theta_deg=0):
    vectors = ctr - center
    angles = np.arctan2(vectors[:,1], vectors[:,0])
    theta = np.radians(theta_deg)
    angles = np.mod(angles - theta, 2 * np.pi) + theta
    sort_idx = np.argsort(angles)
    return ctr[sort_idx]

def resort_atom(area, theta_deg):
    # 以area[i]的四个点与其中心的连线与x轴正方向的夹角进行排序(逆时针为正)
    center = np.mean(area, axis=0)
    vectors = area - center
    angles = np.arctan2(vectors[:,1], vectors[:,0])
    #print(angles*180/np.pi)
    # 若给定一个角度theta，将角度限制在theta到theta+360之间
    # 这里假设theta以弧度为单位
    theta = np.radians(theta_deg)
    angles = np.mod(angles - theta, 2 * np.pi) + theta
    #print(angles*180/np.pi)
    sort_idx = np.argsort(angles)
    return area[sort_idx]

import time
plt.rcParams['font.sans-serif'] = ['Microsoft YaHei', 'SimHei', 'SimSun', 'DejaVu Sans']
plt.rcParams['axes.unicode_minus'] = False  # 解决负号显示问题
def intensity_based_spatial_correlation(data, intensity_threshold=0.1, max_distance=None, bins=50, verbose=True, plot=True):
    """
    统计二维数组中强度相近的像素点之间的空间距离分布
    
    参数:
    - data: 二维数组，data[i,j]表示位置(i,j)处的强度
    - intensity_threshold: 强度相似性阈值，默认0.1
    - max_distance: 最大考虑距离，默认None表示使用图像对角线长度
    - bins: 距离分箱数量，默认50
    - verbose: 是否显示进度信息，默认True
    
    返回:
    - distances: 所有满足条件的点对距离
    - hist: 距离直方图计数
    - bin_edges: 距离分箱边界
    """
    if verbose:
    ...
    # 找出所有强度相似的像素点对
    similar_pairs = []
    # 使用更高效的方法：先找到强度相似的像素点索引
    for idx1 in range(len(intensities)):
        # 找到与当前点强度相似的点的索引
        mask = np.abs(intensities - intensities[idx1]) <= intensity_threshold
        similar_indices_current = np.where(mask)[0]
        # 只保留索引大于当前索引的点，避免重复计数
        similar_indices_current = similar_indices_current[similar_indices_current > idx1]
        for idx2 in similar_indices_current:
            similar_pairs.append((idx1, idx2))
    if verbose:
        print(f"找到 {len(similar_pairs)} 对强度相似的像素点")
    # 计算这些点对之间的距离
    distances = []
    ...
    if plot==True:
        plot_results(hist, bin_edges)
    return distances, hist, bin_edges

def plot_results(hist, bin_edges, title="基于强度的空间距离分布"):
    """
    绘制距离分布结果
    ...
    plt.xlabel('空间距离 (像素)', fontsize=12)
    plt.ylabel('相似点对数量', fontsize=12)
...
def loadmat(file_path):
    """
    以只读方式加载 .mat 文件（HDF5 格式）并将数据复制到字典中，不修改原始文件
    ...
    --------
    基本使用：
        # 以只读方式加载原始文件（不会修改原始文件）
        data = loadmat('original.mat')
        ...
        # 修改数据
        data['dp'] = new_data
        data['ADU'] = np.float32(151)
        
        # 保存到新文件
        savemat('new_file.mat', data)
    
    Note
    ----
    - 此函数以只读模式打开文件，不会修改原始文件
    - 所有数据都被复制到内存中，返回的是数据副本
    - 可以安全地修改返回的字典，不会影响原始文件
    - 原始文件在使用后会自动关闭
    """
    data_dict = {}
    '''
    with h5py.File(file_path, 'r') as file:
        for key in file.keys():
            ...
            if isinstance(dataset, h5py.Dataset):
                print(f"数据集 {key}: 压缩={dataset.compression}, chunks={dataset.chunks}")
    '''
    # 以只读模式打开文件
    with h5py.File(file_path, 'r') as file:
        # 递归读取所有数据集和组
        def read_item(item, name):
            """递归读取数据集和组"""
            if isinstance(item, h5py.Dataset):
                # 如果是数据集，读取数据到内存
                return item[...]  # 读取所有数据到内存
            elif isinstance(item, h5py.Group):
                # 如果是组，递归读取组内的所有项目
                group_dict = {}
                for key in item.keys():
                    group_dict[key] = read_item(item[key], key)
                return group_dict
        
        # 读取所有顶级数据集
        for key in file.keys():
            data_dict[key] = read_item(file[key], key)
    
    return data_dict

def savemat(output_file_path, data_dict, overwrite=False):
    """
    将字典中的数据保存到新的 .mat 文件（MATLAB v7.3 HDF5 格式），不覆盖原始文件
    ...
    output_file_path : str
        输出 .mat 文件的路径
    data_dict : dict
        包含数据的字典（从 loadmat 函数返回）
        - 键：数据集名称（字符串）
        - 值：numpy 数组或字典（如果是嵌套结构）
    overwrite : bool, optional
        如果输出文件已存在，是否覆盖。默认为 False（不覆盖，会抛出错误）
    ...
    覆盖已存在的文件：
        savemat('new_file.mat', data, overwrite=True)
    
    Note
    ----
    - 此函数会创建一个新的 .mat 文件，不会修改原始文件
    - 如果 output_file_path 已存在且 overwrite=False，会抛出 FileExistsError
    - 支持嵌套的字典结构（组）
    - dp 数据集会使用 gzip 压缩，其他数据集不使用压缩
    - 保存的文件与 MATLAB v7.3 格式兼容
    - 此函数会自动关闭新创建的文件
    """
    import os
    ...
    if os.path.exists(output_file_path) and not overwrite:
        raise FileExistsError(
            f"输出文件 '{output_file_path}' 已存在。"
            "如果要覆盖，请设置 overwrite=True"
        )
    
    # 创建新的 HDF5 文件（MATLAB v7.3 格式）
    with h5py.File(output_file_path, 'w') as output_file:
        # 创建 MATLAB 引用组（即使为空，MATLAB 也需要这个组）
        if '#refs#' not in output_file.keys():
            output_file.create_group('#refs#')
        
        # 递归写入数据
        def write_item(dest, name, value):
            """递归写入数据集和组，添加 MATLAB 兼容属性"""
            if isinstance(value, dict):
                # 如果是字典，创建组并递归写入
                group = dest.create_group(name)
                for key, val in value.items():
                    write_item(group, key, val)
            elif isinstance(value, np.ndarray):
                # 如果是 numpy 数组，创建数据集
                # MATLAB 使用列优先存储，但 h5py 默认使用行优先
                # 我们需要保持原始维度顺序，但添加 MATLAB 属性
                ...
                matlab_class = dtype_map.get(value.dtype.type, 'double')
                
                # 根据原始文件检查结果，为不同数据集应用相应压缩
                if name == 'dp':
                    # dp 数据集使用 gzip 压缩和分块
                    dset = dest.create_dataset(name, data=value, dtype=value.dtype,
                                             compression='gzip', chunks=(1, 256, 32))
                else:
                    # 其他数据集不使用压缩
                    dset = dest.create_dataset(name, data=value, dtype=value.dtype)
                
                # 添加 MATLAB 兼容属性（MATLAB 期望以 null 结尾的字符串）
                matlab_class_bytes = matlab_class.encode('ascii') + b'\x00'
                dset.attrs['MATLAB_class'] = np.frombuffer(matlab_class_bytes, dtype=np.uint8)
                
            elif isinstance(value, (int, float, np.integer, np.floating)):
                # 如果是标量，转换为 numpy 数组后创建数据集
                ...
                try:
                    ...
                    dset = dest.create_dataset(name, data=value, dtype=value.dtype)
                    # 添加 MATLAB 兼容属性（MATLAB 期望以 null 结尾的字符串）
                    matlab_class = 'double'  # 默认
                    matlab_class_bytes = matlab_class.encode('ascii') + b'\x00'
                    dset.attrs['MATLAB_class'] = np.frombuffer(matlab_class_bytes, dtype=np.uint8)
                except Exception as e:
                    raise TypeError(f"无法保存类型为 {type(value)} 的数据 '{name}': {e}")
        
        # 写入所有顶级数据集
        for key, value in data_dict.items():
            if key != '#refs#':  # 跳过 #refs# 组，我们已经创建了它
                write_item(output_file, key, value)
    
    print(f"数据已保存到: {output_file_path}")

def savemat_preserve_format(output_file_path, data_dict, reference_file_path=None, overwrite=False):
    """
    将字典中的数据保存到新的 .mat 文件，并尽可能保持与参考文件相同的格式
    ...
    reference_file_path : str, optional
        参考文件的路径。如果提供，将从参考文件读取元数据（形状、类型、属性等）
        并应用到新文件，以保持格式一致。
    overwrite : bool, optional
        如果输出文件已存在，是否覆盖。默认为 False
    ...
    基本使用：
        ...
        # 保存并保持原始格式
        savemat_preserve_format('new_file.mat', data, reference_file_path='original.mat')
    """
    import os
    ...
    if reference_file_path and os.path.exists(reference_file_path):
        try:
            with h5py.File(reference_file_path, 'r') as ref_file:
            ...
            print(f"警告: 无法读取参考文件元数据: {e}")
            metadata = {}
    
    # 创建新的 HDF5 文件（MATLAB v7.3 格式）
    with h5py.File(output_file_path, 'w') as output_file:
        # 创建 MATLAB 引用组（即使为空，MATLAB 也需要这个组）
        # 但只有在参考文件有 #refs# 组时才创建
        if reference_file_path and os.path.exists(reference_file_path):
            try:
                with h5py.File(reference_file_path, 'r') as ref_file:
                    if '#refs#' in ref_file.keys():
                        output_file.create_group('#refs#')
            except:
                pass
        
        # 递归写入数据
        def write_item(dest, name, value, meta=None):
            """递归写入数据集和组，使用元数据保持格式一致"""
            if isinstance(value, dict):
                # 如果是字典，创建组并递归写入
                group = dest.create_group(name)
                for key, val in value.items():
                ...
                if meta is not None:
                    ...
                    # 确保形状匹配（对于标量，原始可能是 (1,1)）
                    if meta['shape'] != value.shape:
                        if meta['shape'] == (1, 1):
                            # 原始是 (1,1)，转换为 (1,1)
                            if value.size == 1:
                                value = np.array([[value.item()]], dtype=target_dtype)
                            else:
                                # 如果值已经是数组，取第一个元素
                                value = np.array([[value.flat[0]]], dtype=target_dtype)
                        elif len(meta['shape']) == 0:
                            # 原始是标量，保持为标量
                            if value.size == 1:
                                value = np.array(value.item(), dtype=target_dtype)
                            else:
                            ...
                            # 其他情况，尝试 reshape（如果元素数量匹配）
                            if value.size == np.prod(meta['shape']):
                                value = value.reshape(meta['shape']).astype(target_dtype)
                            # 如果形状不匹配且无法 reshape，保持原样（可能是新数据）
                    
                    # 使用元数据的压缩设置
                    if name == 'dp' or meta['compression']:
                        compression = meta['compression'] if meta['compression'] else 'gzip'
                        chunks = meta['chunks'] if meta['chunks'] else (1, 256, 32)
                        dset = dest.create_dataset(name, data=value, dtype=target_dtype,
                ...
                if meta is not None:
                    ...
                    # 转换为正确的形状和类型
                    if target_shape == (1, 1):
                        value = np.array([[float(value)]], dtype=target_dtype)
                    elif len(target_shape) == 0:
                        value = np.array(value, dtype=target_dtype)
                    else:
                        value = np.array(value, dtype=target_dtype).reshape(target_shape)
        ...
        # 写入所有顶级数据集
        for key, value in data_dict.items():
            if key != '#refs#':  # 跳过 #refs# 组
                meta = metadata.get(key, None)
                write_item(output_file, key, value, meta)
    
    print(f"数据已保存到: {output_file_path} (已保持参考文件格式)")
    
from scipy.stats import pearsonr
from scipy.spatial.distance import cosine
def Intensity_similarity(value_all, outpath):
    # =============================================================================
    # 相似度评估方法
    # =============================================================================
    def calculate_similarity_group1_group2(group1, group2, method='pearson'):
        """
        计算两组有序数据（每组6个数字）的相似度
        
        参数:
            group1: 第一组数据，形状为 (6,) 的数组
            group2: 第二组数据，形状为 (6,) 的数组
            method: 相似度计算方法
                - 'pearson': 皮尔逊相关系数 (范围: -1到1, 1表示完全正相关)
                - 'cosine': 余弦相似度 (范围: 0到1, 1表示完全相同方向)
                - 'euclidean': 欧氏距离 (范围: 0到无穷, 0表示完全相同，值越小越相似)
                - 'manhattan': 曼哈顿距离 (范围: 0到无穷, 0表示完全相同，值越小越相似)
                - 'mse': 均方误差 (范围: 0到无穷, 0表示完全相同，值越小越相似)
                - 'rmse': 均方根误差 (范围: 0到无穷, 0表示完全相同，值越小越相似)
        
        返回:
            相似度值（对于距离类方法，返回值越小表示越相似）
        """
        group1 = np.array(group1).flatten()
        group2 = np.array(group2).flatten()
        
        if method == 'pearson':
            # 皮尔逊相关系数：衡量线性相关性，适合比较趋势
            corr, _ = pearsonr(group1, group2)
            return corr  # 范围 [-1, 1]，1表示完全正相关
        elif method == 'cosine':
            # 余弦相似度：衡量方向相似性，不受幅度影响
            cos_sim = 1 - cosine(group1, group2)  # cosine返回距离，转换为相似度
            return cos_sim  # 范围 [0, 1]，1表示完全相同方向
        elif method == 'euclidean':
            # 欧氏距离：衡量绝对差异
            dist = np.sqrt(np.sum((group1 - group2)**2))
            return dist  # 范围 [0, ∞)，0表示完全相同
        elif method == 'manhattan':
            # 曼哈顿距离：逐元素绝对差之和
            dist = np.sum(np.abs(group1 - group2))
            return dist  # 范围 [0, ∞)，0表示完全相同
        elif method == 'mse':
            # 均方误差
            mse = np.mean((group1 - group2)**2)
            return mse  # 范围 [0, ∞)，0表示完全相同
        elif method == 'rmse':
            # 均方根误差
            rmse = np.sqrt(np.mean((group1 - group2)**2))
            return rmse  # 范围 [0, ∞)，0表示完全相同
        else:
            raise ValueError(f"未知的方法: {method}")

    # 示例：比较两组数据的相似度
    # 假设 value_all 的形状为 (2, 6, height, width)
    # 对于每个像素位置，比较 T=0 和 T=1 的6个强度值的相似度

    if len(value_all) >= 2:
        # 提取两组数据：T=0 和 T=1
        group1 = value_all[0]  # 形状: (6, height, width)
        group2 = value_all[1]  # 形状: (6, height, width)
        height, width = group1.shape[1], group1.shape[2]
        # 创建相似度映射（针对每个像素位置计算相似度）
        similarity_map_pearson = np.zeros((height, width))
        similarity_map_cosine = np.zeros((height, width))
        similarity_map_euclidean = np.zeros((height, width))
        similarity_map_manhattan = np.zeros((height, width))
        similarity_map_mse = np.zeros((height, width))
        similarity_map_rmse = np.zeros((height, width))
        for i in range(height):
            for j in range(width):
                g1 = group1[:, i, j]
                g2 = group2[:, i, j]
                similarity_map_pearson[i, j] = calculate_similarity_group1_group2(g1, g2, method='pearson')
                similarity_map_cosine[i, j] = calculate_similarity_group1_group2(g1, g2, method='cosine')
                similarity_map_euclidean[i, j] = calculate_similarity_group1_group2(g1, g2, method='euclidean')
                similarity_map_manhattan[i, j] = calculate_similarity_group1_group2(g1, g2, method='manhattan')
                similarity_map_mse[i, j] = calculate_similarity_group1_group2(g1, g2, method='mse')
                similarity_map_rmse[i, j] = calculate_similarity_group1_group2(g1, g2, method='rmse')
        # 全部归一化（归一化每一种相似度/距离到0-1范围，便于对比）
        def normalize_map(map_):
            min_val = np.nanmin(map_)
            max_val = np.nanmax(map_)
            if max_val - min_val == 0:
                return np.zeros_like(map_)
            return (map_ - min_val) / (max_val - min_val)
            #return 

        similarity_map_pearson   = normalize_map(similarity_map_pearson)
        similarity_map_cosine    = normalize_map(similarity_map_cosine)
        similarity_map_euclidean = normalize_map(similarity_map_euclidean)
        similarity_map_manhattan = normalize_map(similarity_map_manhattan)
        similarity_map_mse       = normalize_map(similarity_map_mse)
        similarity_map_rmse      = normalize_map(similarity_map_rmse)

        # 对于欧氏距离，值越小越相似，可以转换为相似度 (可选)
        # similarity_map_euclidean_sim = 1 / (1 + similarity_map_euclidean)
        print(f"皮尔逊相关系数范围: [{similarity_map_pearson.min():.3f}, {similarity_map_pearson.max():.3f}]")
        print(f"余弦相似度范围: [{similarity_map_cosine.min():.3f}, {similarity_map_cosine.max():.3f}]")
        print(f"欧氏距离范围: [{similarity_map_euclidean.min():.3f}, {similarity_map_euclidean.max():.3f}]")
        print(f"曼哈顿距离范围: [{similarity_map_manhattan.min():.3f}, {similarity_map_manhattan.max():.3f}]")
        print(f"均方误差范围: [{similarity_map_mse.min():.3f}, {similarity_map_mse.max():.3f}]")
        print(f"均方根误差范围: [{similarity_map_rmse.min():.3f}, {similarity_map_rmse.max():.3f}]")
        # 可视化相似度映射
        fig, axes = plt.subplots(2, 3, figsize=(15, 10))
        # 统一控制字体和粗细
        fontdict_title = {'fontsize': 14, 'fontweight': 'bold', 'family': 'Arial'}
        fontdict_label = {'fontsize': 12, 'fontweight': 'normal', 'family': 'Arial'}
        for ax in axes.flat:
            ax.tick_params(labelsize=10)
            # 控制坐标轴字体
            ax.set_xlabel(ax.get_xlabel(), fontdict=fontdict_label)
            ax.set_ylabel(ax.get_ylabel(), fontdict=fontdict_label)

        im1 = axes[0, 0].imshow(similarity_map_pearson, cmap='RdYlGn', vmin=-1, vmax=1)
        axes[0, 0].set_title('Pearson Correlation\n(higher = more similar)', fontdict=fontdict_title)
        plt.colorbar(im1, ax=axes[0, 0])
        
        im2 = axes[0, 1].imshow(similarity_map_cosine, cmap='RdYlGn', vmin=0, vmax=1)
        axes[0, 1].set_title('Cosine Similarity\n(higher = more similar)', fontdict=fontdict_title)
        plt.colorbar(im2, ax=axes[0, 1])
        
        im3 = axes[0, 2].imshow(similarity_map_euclidean, cmap='RdYlGn_r')  # _r表示反转colormap
        axes[0, 2].set_title('Euclidean Distance\n(lower = more similar)', fontdict=fontdict_title)
        plt.colorbar(im3, ax=axes[0, 2])

        im4 = axes[1, 0].imshow(similarity_map_manhattan, cmap='RdYlGn_r')
        axes[1, 0].set_title('Manhattan Distance\n(lower = more similar)', fontdict=fontdict_title)
        plt.colorbar(im4, ax=axes[1, 0])

        im5 = axes[1, 1].imshow(similarity_map_mse, cmap='RdYlGn_r')
        axes[1, 1].set_title('Mean Squared Error\n(lower = more similar)', fontdict=fontdict_title)
        plt.colorbar(im5, ax=axes[1, 1])

        im6 = axes[1, 2].imshow(similarity_map_rmse, cmap='RdYlGn_r')
        axes[1, 2].set_title('Root Mean Squared Error\n(lower = more similar)', fontdict=fontdict_title)
        plt.colorbar(im6, ax=axes[1, 2])
        
        plt.tight_layout()
        os.makedirs(outpath, exist_ok=True)
        plt.savefig(f'{outpath}/similarity_comparison.png', dpi=300, bbox_inches='tight')
        plt.show()
        
        # 保存相似度映射
        np.savetxt(f'{outpath}/similarity_pearson.dat', similarity_map_pearson)
        np.savetxt(f'{outpath}/similarity_cosine.dat', similarity_map_cosine)
        np.savetxt(f'{outpath}/similarity_euclidean.dat', similarity_map_euclidean)
        np.savetxt(f'{outpath}/similarity_manhattan.dat', similarity_map_manhattan)
        np.savetxt(f'{outpath}/similarity_mse.dat', similarity_map_mse)
        np.savetxt(f'{outpath}/similarity_rmse.dat', similarity_map_rmse)
        
    results = [similarity_map_pearson, similarity_map_cosine, similarity_map_euclidean, 
               similarity_map_manhattan, similarity_map_mse, similarity_map_rmse]
    # 如果需要只比较两组数据（不针对每个像素），可以使用以下代码：
    # group1_flat = value_all[0].mean(axis=(1,2))  # 平均所有像素，得到6个值
    # group2_flat = value_all[1].mean(axis=(1,2))  # 平均所有像素，得到6个值
    # similarity = calculate_similarity_group1_group2(group1_flat, group2_flat, method='pearson')
    # print(f"整体相似度 (Pearson): {similarity:.4f}")
    return results

def SNR_analysis(test_position, blob, width, plot=False):
    x=int(blob[1])
    y=int(blob[0])
    print(test_position.shape)
    profile_all = []
    for i in range(test_position.shape[0]):
        for j in range(test_position.shape[1]):
            profile = test_position[i,j,y,x-width:x+width+1]
            profile_all.append(profile)
            #AD.autodisk.saveData(f'{folder}/{out}/ED-pattern.dat',test_position[0,0],dataformat='np.float32', overwrite=True)
            if plot==True:
                plt.figure(figsize=(10, 5))
                plt.plot(profile, marker='o', linestyle='-')
                plt.title('Profile Visualization')
                plt.xlabel('Position')
                plt.ylabel('Intensity')
                plt.grid()
                plt.axhline(0, color='black', linewidth=0.5, linestyle='--')
                plt.show()
    profile_all = np.array(profile_all)
    print(profile_all.shape)
    num_mean_std_SNR_all=[]
    for num in range(profile_all.shape[1]):
        mean = np.mean(profile_all[:,num])
        std_dev = np.std(profile_all[:,num])
        SNR = mean/std_dev
        num_mean_std_SNR_all.append([num+1, round(mean, 2), round(std_dev, 2), round(SNR, 2)])
        print(f'{num}:', f'Mean: {mean:.2f}', f'Standard Deviation: {std_dev:.2f}', f'SNR: {SNR}')
        if plot==True:
            plt.figure(figsize=(10, 5))
            plt.hist(profile_all[:,num], bins=30, color='blue', alpha=0.7, edgecolor='black')
            ...
            plt.grid()
            plt.show()
    return profile, np.array(num_mean_std_SNR_all)

def create_ring_matrix(x, y, size, radius, ring_width):
    """
    创建一个指定大小的矩阵，指定半径和圆环宽度
    :param size: 矩阵的大小 (行, 列)
    :param radius: 圆环的内半径
    :param ring_width: 圆环的宽度
    :return: 带有圆环的矩阵
    """
    ...
    for i in range(size[0]):
        for j in range(size[1]):
            ...
            if radius <= distance < outer_radius:
                matrix[i, j] = 1
    return matrix
